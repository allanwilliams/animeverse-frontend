...
import type { AnimeList, Genero } from '@/types/anime';
...
interface AnimesClientProps {
  ...
  initialUltimoEpisodioMap?: Record<number, number | null>;
}

export function AnimesClient({
  initialData,
  initialGeneros,
  initialPage = 1,
  initialSearch = '',
  ...
  const updateURL = useCallback((search: string, generos: string[], statuses: string[], page: number) => {
    ...
    if (generos.length > 0) {
      generos.forEach(genero => params.append('genero', genero));
    }
    ...
    const newURL = queryString ? `/animes?${queryString}` : '/animes';
    router.push(newURL);
  }, [router]);
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-[2]">
                <SearchInput
                  ...
                  placeholder="Buscar por título, estúdio..."
                />
              </div>
              ...
              <div className="flex-1">
                <StatusAutocomplete selectedStatuses={tempStatuses} onStatusChange={handleStatusChange} />
              </div>
            </div>
        ...
        {loading ? (
          <div className="flex justify-center py-12">
            <Loading size="lg" text="Carregando animes..." />
          </div>
        ) : (
          <>
            <div className="mb-4 text-gray-400">
              {data && <p>{data.count} anime(s) encontrado(s)</p>}
            </div>
            
            <AnimeGrid animes={data?.results || []} ultimoEpisodioMap={initialUltimoEpisodioMap} />
